...
def run() -> pd.DataFrame:
    ...
    write_table(frame, "training_matrix_daily_predictable", PROCESSED_DIR / "training_matrix_daily_predictable.csv")
    try:
        from ..data_qa import build_training_matrix_source_audit, source_priority_policy

        source_priority_policy()
        build_training_matrix_source_audit()
    except Exception:
        # The matrix itself is the authoritative output; source-audit sidecars are regenerated by qa-data/freeze.
        pass
    return frame
